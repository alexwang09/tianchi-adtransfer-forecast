...
def get_min_query_time_gap_before(s):
    ...
    current_time1 = timestamp_datetime(int(current_time))
    day = current_time1[8:10]
...
def base_process(data):
    ...
    data['age0'] = data['user_age_level'].apply(lambda x: 1 if x == 1004 or x == 1005 or x == 1006 or x == 1007  else 2)
    data['occupation0'] = data['user_occupation_id'].apply(lambda x: 1 if x == -1 or x == 2003  else 2)
    data['star0'] = data['user_star_level'].apply(lambda x: 1 if x == -1 or x == 3000 or x == 3001  else 2)
    print(
        '--------------------------------------------------------------context--------------------------------------------------------------')
    ...
    data['context_page0'] = data['context_page_id'].apply(
        lambda x: 1 if x == 4001 or x == 4002 or x == 4003 or x == 4004 or x == 4007  else 2)
    print(
        '--------------------------------------------------------------shop--------------------------------------------------------------')
    for col in ['shop_id']:
        data[col] = lbl.fit_transform(data[col])
...
def slide_cnt(data):
    ...
    print len(data)
    print len(data.instance_user_id.unique())
    print('当前日期前一天的cnt')
    for d in range(19, 26):  # 18到24号
        df1 = data[data['day'] == d - 1]
        traded_df1 = df1[df1.is_trade == 1]
        df2 = data[data['day'] == d]  # 19到25号
        user_cnt = df1.groupby(by='user_id').agg('size').to_dict()
        item_cnt = df1.groupby(by='item_id').agg('size').to_dict()
        shop_cnt = df1.groupby(by='shop_id').agg('size').to_dict()
        user_sale = df1.groupby(by='user_id').agg('size').to_dict()
        item_sale = df1.groupby(by='item_id').agg('size').to_dict()
        shop_sale = df1.groupby(by='shop_id').agg('size').to_dict()
        ...
        df2['user_sale1'] = df2['user_id'].apply(lambda x: user_sale.get(x, 0))
        df2['item_sale1'] = df2['item_id'].apply(lambda x: item_sale.get(x, 0))
        df2['shop_sale1'] = df2['shop_id'].apply(lambda x: shop_sale.get(x, 0))
        df2['user_rate1'] = df2.user_sale1/df2.user_cnt1
        df2['item_rate1'] = df2.item_sale1/df2.item_cnt1
        df2['shop_rate1'] = df2.shop_sale1/df2.shop_cnt1
        df2 = df2[['user_cnt1', 'item_cnt1', 'shop_cnt1', 'instance_user_id',
                'user_sale1', 'item_sale1', 'shop_sale1',
                'user_rate1', 'item_rate1', 'shop_rate1']]
        if d == 19:
            Df2 = df2
        else:
            Df2 = pd.concat([df2, Df2])
    data = pd.merge(data, Df2, on=['instance_user_id'], how='left')
    '''
    print('当前日期之前的cnt')
    for d in range(19, 26):
        ...
        df2['user_salex'] = df2['user_id'].apply(lambda x: user_sale.get(x, 0))
        df2['item_salex'] = df2['item_id'].apply(lambda x: item_sale.get(x, 0))
        df2['shop_salex'] = df2['shop_id'].apply(lambda x: shop_sale.get(x, 0))
        df2['user_ratex'] = df2.user_salex/df2.user_cntx
        df2['item_ratex'] = df2.item_salex/df2.item_cntx
        df2['shop_ratex'] = df2.shop_salex/df2.shop_cntx

        df2 = df2[['user_cntx', 'item_cntx', 'shop_cntx', 'instance_user_id',
                'user_salex', 'item_salex', 'shop_salex',
                'user_ratex', 'item_ratex', 'shop_ratex']]
        if d == 19:
            Df2 = df2
        else:
            Df2 = pd.concat([df2, Df2])
...
def thisday(data):
    ...
    t1 = data.groupby(['user_id', 'day']).agg('size').reset_index().rename(columns={0: 'this_day_user_query_count'})
    t2 = data.groupby(['user_id', 'day', 'hour']).agg('size').reset_index().rename(columns={0: 'this_day_hour_user_query_count'})
    t3 = data.groupby(['user_id', 'day', 'shop_id']).agg('size').reset_index().rename(columns={0: 'this_day_user_query_same_shop_count'})
    t4 = data.groupby(['user_id', 'day', 'item_id']).agg('size').reset_index().rename(columns={0: 'this_day_user_query_same_item_count'})
    t5 = data.groupby(['shop_id', 'day']).agg('size').reset_index().rename(columns={0: 'this_day_shop_query_count'})

    data = pd.merge(data, t1, on=['user_id', 'day'], how='left')
    data = pd.merge(data, t2, on=['user_id', 'day', 'hour'], how='left')
    data = pd.merge(data, t3, on=['user_id', 'day', 'shop_id'], how='left')
    data = pd.merge(data, t4, on=['user_id', 'day', 'item_id'], how='left')
    data = pd.merge(data, t5, on=['shop_id', 'day'], how='left')
...
def item(data):
    ...
    itemcnt = data.groupby(['item_id'], as_index=False)['instance_user_id'].agg({'item_cnt': 'count'})
    ...
    for col in ['item_brand_id','item_city_id', 'item_price_level', 'item_sales_level', 'item_collected_level', 'item_pv_level']:
        itemcnt = data.groupby([col, 'item_id'], as_index=False)['instance_user_id'].agg({str(col) + '_item_cnt': 'count'})
    ...
    itemcnt = data.groupby(['item_brand_id'], as_index=False)['instance_user_id'].agg({'item_brand_cnt': 'count'})
    ...
    for col in ['item_city_id', 'item_price_level', 'item_sales_level', 'item_collected_level', 'item_pv_level']:
        itemcnt = data.groupby([col, 'item_brand_id'], as_index=False)['instance_user_id'].agg({str(col) + '_brand_cnt': 'count'})
    ...
    itemcnt = data.groupby(['item_city_id'], as_index=False)['instance_user_id'].agg({'item_city_cnt': 'count'})
    ...
    for col in ['item_price_level', 'item_sales_level', 'item_collected_level', 'item_pv_level']:
        itemcnt = data.groupby([col, 'item_city_id'], as_index=False)['instance_user_id'].agg({str(col) + '_city_cnt': 'count'})
    ...
    itemcnt = data.groupby(['item_price_level'], as_index=False)['instance_user_id'].agg({'item_price_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['item_price_level'], how='left')
    for col in ['item_sales_level', 'item_collected_level', 'item_pv_level']:
        itemcnt = data.groupby([col, 'item_city_id'], as_index=False)['instance_user_id'].agg({str(col) + '_price_cnt': 'count'})
        data = pd.merge(data, itemcnt, on=[col, 'item_city_id'], how='left')
        data[str(col) + '_price_prob'] = data[str(col) + '_price_cnt'] / data['item_price_cnt']
    del data['item_price_cnt']

    print('一个item_sales_level有多少item_collecwithted_level，item_pv_level')

    itemcnt = data.groupby(['item_sales_level'], as_index=False)['instance_user_id'].agg({'item_salse_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['item_sales_level'], how='left')
    for col in ['item_collected_level', 'item_pv_level']:
        itemcnt = data.groupby([col, 'item_sales_level'], as_index=False)['instance_user_id'].agg({str(col) + '_salse_cnt': 'count'})
    ...
    itemcnt = data.groupby(['item_collected_level'], as_index=False)['instance_user_id'].agg({'item_coll_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['item_collected_level'], how='left')
    for col in ['item_pv_level']:
        itemcnt = data.groupby([col, 'item_collected_level'], as_index=False)['instance_user_id'].agg({str(col) + '_coll_cnt': 'count'})
...
def user(data):
    print('用户有多少性别')
    itemcnt = data.groupby(['user_id'], as_index=False)['instance_user_id'].agg({'user_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_id'], how='left')

    for col in ['user_gender_id','user_age_level', 'user_occupation_id', 'user_star_level']:
        itemcnt = data.groupby([col, 'user_id'], as_index=False)['instance_user_id'].agg({str(col) + '_user_cnt': 'count'})
        data = pd.merge(data, itemcnt, on=[col, 'user_id'], how='left')
        data[str(col) + '_user_prob']=data[str(col) + '_user_cnt']/data['user_cnt']
    del data['user_cnt']

    print('性别的年龄段，职业有多少')
    itemcnt = data.groupby(['user_gender_id'], as_index=False)['instance_user_id'].agg({'user_gender_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_gender_id'], how='left')

    for col in ['user_age_level', 'user_occupation_id', 'user_star_level']:
        itemcnt = data.groupby([col, 'user_gender_id'], as_index=False)['instance_user_id'].agg({str(col) + '_user_gender_cnt': 'count'})
    ...
    itemcnt = data.groupby(['user_age_level'], as_index=False)['instance_user_id'].agg({'user_age_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_age_level'], how='left')

    for col in ['user_occupation_id', 'user_star_level']:
        itemcnt = data.groupby([col, 'user_age_level'], as_index=False)['instance_user_id'].agg({str(col) + '_user_age_cnt': 'count'})
    ...
    itemcnt = data.groupby(['user_occupation_id'], as_index=False)['instance_user_id'].agg({'user_occ_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['user_occupation_id'], how='left')
    for col in ['user_star_level']:
        itemcnt = data.groupby([col, 'user_occupation_id'], as_index=False)['instance_user_id'].agg({str(col) + '_user_occ_cnt': 'count'})
...
def user_item(data):
    ...
    for col in ['item_id',
        ...
        item_shop_cnt = data.groupby([col, 'user_id'], as_index=False)['instance_user_id'].agg({str(col)+'_user_cnt': 'count'})
    ...
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
                'item_sales_level','item_collected_level','item_pv_level']:
        item_shop_cnt = data.groupby([col, 'user_gender_id'], as_index=False)['instance_user_id'].agg({str(col)+'_user_gender_cnt': 'count'})
    ...
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
                'item_sales_level','item_collected_level','item_pv_level']:
        item_shop_cnt = data.groupby([col, 'user_age_level'], as_index=False)['instance_user_id'].agg({str(col)+'_user_age_cnt': 'count'})
    ...
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
                'item_sales_level','item_collected_level','item_pv_level']:
        item_shop_cnt = data.groupby([col, 'user_occupation_id'], as_index=False)['instance_user_id'].agg({str(col)+'_user_occ_cnt': 'count'})
...
def user_shop(data):
    ...
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
        item_shop_cnt = data.groupby([col, 'user_id'], as_index=False)['instance_user_id'].agg(
    ...
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
        item_shop_cnt = data.groupby([col, 'user_gender_id'], as_index=False)['instance_user_id'].agg(
    ...
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
        item_shop_cnt = data.groupby([col, 'user_age_level'], as_index=False)['instance_user_id'].agg(
    ...
    for col in ['shop_id', 'shop_review_num_level', 'shop_star_level']:
        item_shop_cnt = data.groupby([col, 'user_occupation_id'], as_index=False)['instance_user_id'].agg(
...
def shop_item(data):
    ...
    itemcnt = data.groupby(['shop_id'], as_index=False)['instance_user_id'].agg({'shop_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['shop_id'], how='left')
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
                'item_sales_level','item_collected_level','item_pv_level']:
        item_shop_cnt = data.groupby([col, 'shop_id'], as_index=False)['instance_user_id'].agg({str(col)+'_shop_cnt': 'count'})
    ...
    itemcnt = data.groupby(['shop_review_num_level'], as_index=False)['instance_user_id'].agg({'shop_rev_cnt': 'count'})
    data = pd.merge(data, itemcnt, on=['shop_review_num_level'], how='left')
    for col in ['item_id',
                'item_brand_id','item_city_id','item_price_level',
                'item_sales_level','item_collected_level','item_pv_level']:
        item_shop_cnt = data.groupby([col, 'shop_review_num_level'], as_index=False)['instance_user_id'].agg({str(col)+'_shop_rev_cnt': 'count'})
    ...
    # itemcnt = data.groupby(['shop_star_level'], as_index=False)['instance_user_id'].agg({'shop_star_cnt': 'count'})
    # data = pd.merge(data, itemcnt, on=['shop_star_level'], how='left')
    # for col in ['item_id',
    #             'item_brand_id', 'item_city_id', 'item_price_level',
    #             'item_sales_level', 'item_collected_level', 'item_pv_level']:
    #     item_shop_cnt = data.groupby([col, 'shop_star_level'], as_index=False)['instance_user_id'].agg({str(col) + '_shop_star_cnt': 'count'})
...
def lgbCV(train, test):
    ...
    col = [c for c in train if
           c not in ['is_trade', 'item_category_list', 'item_property_list', 'predict_category_property', 'instance_id',
                     'context_id', 'realtime', 'context_timestamp', 'item_id', 'user_id', 'shop_id', 'item_brand_id', 'instance_user_id']]
    ...
    lgb0 = lgb.LGBMClassifier(
        ...
        subsample=0.9,
        n_estimators=20000)
    lgb_model = lgb0.fit(X, y, eval_set=[(X, y), (X_tes, y_tes)], early_stopping_rounds=200)
    ...
    print sorted(zip(lgb_model.feature_importances_, col))[::-1]
    test['pred'] = pred
    test['index'] = range(len(test))
    # print(test[['is_trade','pred']])
    print(u'误差', log_loss(test['is_trade'], test['pred']))
    return best_iter
...
if __name__ == "__main__":
    ...
    data = data.drop_duplicates().reset_index(drop=True)  # 把instance id去重
